...
export default function PeminjamanUser() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-100">
        ...
        <div className="flex items-center gap-3 mb-10 cursor-pointer">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-sm text-gray-300">
              {user?.role === "user" ? "Siswa" : user?.role || "Siswa"}
            </p>
          </div>
        </div>
      ...
      {/* MAIN */}
      <main
        className={`flex-1 p-6 w-full mt-20 transition-all duration-300 ${
          open ? "ml-72" : "ml-0"
        }`}
      >
          ...
          <div className="bg-white rounded-xl shadow p-4 flex gap-4 items-start">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 text-sm">
                <p><span className="font-medium">Tanggal Pinjam:</span> {borrowedBook.pinjam}</p>
                <p><span className="font-medium">Batas Kembali:</span> {borrowedBook.kembali}</p>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-80 text-center">
            <h2 className="font-bold text-lg mb-3 text-blue-900">Konfirmasi Pengembalian</h2>
            <p className="text-gray-700 mb-5">
              Apakah kamu yakin ingin mengembalikan buku ini?
            </p>

            <div className="flex gap-4">
              <button onClick={() => setShowModal(false)} className="flex-1 bg-gray-300 py-2 rounded-lg">
                Batal
              </button>
              <button onClick={handleKonfirmasi} className="flex-1 bg-blue-900 text-white py-2 rounded-lg">
                Ya, Kembalikan
              </button>
            </div>
          </div>
