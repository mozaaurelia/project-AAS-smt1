...
export default function Pengembalian() {
  ...
  // Fetch buku yang dipinjam
  const fetchPeminjaman = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/peminjaman"); // pastikan API ini sudah ada
  ...
  const handleKembalikan = async (id) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // Refresh list setelah mengembalikan buku
        fetchPeminjaman();
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {peminjamanData.map((item) => (
              ...
              >
                <img
                  src={item.gambar ? `/image/${item.gambar}` : "/image/no-image.jpg"}
                  alt={item.judul}
                  className="w-40 h-52 object-cover rounded-lg mb-4"
                />
                <h3 className="font-semibold text-base text-center">{item.judul}</h3>
                <p className="text-sm text-gray-600 mb-4">{item.pengarang}</p>
                <button
